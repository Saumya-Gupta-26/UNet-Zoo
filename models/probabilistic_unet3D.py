...
from  torchlayers import Conv3D, Conv3DSequence, ReversibleSequence
    ...
    def __init__(self,
        ...
        for i in range(len(self.num_filters)):
            ...
            if reversible:
                ...
                layers.append(Conv3DSequence(input_dim, output_dim, kernel=3, depth=no_convs_per_block))
    ...
    def forward(self, input, segm=None):
        if segm is not None:
            with torch.no_grad():
                ...
                segm_one_hot = segm_one_hot.float() # torch.Size([1, 2, 128, 128, 128])
...
class Fcomb(nn.Module):
    ...
    def __init__(self, num_filters, latent_dim, num_output_channels, num_classes, no_convs_fcomb, initializers,
        ...
        if self.use_tile:
            ...
            layers.append(Conv3D(self.num_filters[0] + self.latent_dim, self.num_filters[0], kernel_size=1))

            for _ in range(no_convs_fcomb - 2):
                layers.append(Conv3D(self.num_filters[0], self.num_filters[0], kernel_size=1))
    ...
    def forward(self, feature_map, z):
        """
        Z is batch_sizexlatent_dim and feature_map is batch_sizexno_channelsxHxWxD.
...
class ProbabilisticUnet3D(nn.Module):
    ...
    def __init__(self, input_channels=1,
        ...
        self.unet = Unet3D(self.input_channels, self.num_classes, self.num_filters, initializers=self.initializers,
        ...
        self.last_conv = Conv3D(32, num_classes, kernel_size=1, activation=torch.nn.Identity, norm=torch.nn.Identity)
