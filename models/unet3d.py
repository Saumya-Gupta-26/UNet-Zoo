...
class UpConvBlock(nn.Module):
    ...
    def forward(self, x, bridge):
        if self.bilinear:
            up = nn.functional.interpolate(x, mode='trilinear', scale_factor=2, align_corners=False) # saumya : need to use trilinear for 3D, not bilinear
        else:
